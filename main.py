...
def predict():
    """Predict sales for the specified month and year."""
    try:
        ...
        if year == 2024:
            # Determine previous months and years
            if month == 'January':
                ...
                check_years = [
                    year if month_index - i > 0 else year - 1
                    for i in range(1, 4)
                ]
            ...
            filtered_data['MonthIndex'] = filtered_data['Month'].map(lambda x: MONTH_ORDER.index(x))
            filtered_data.sort_values(by=['Year', 'MonthIndex'], inplace=True)
            ...
            # Calculate the average sales for prediction
            recent_data = df[
                (df['Product'] == product) &
                (df['Volume'] == volume) &
